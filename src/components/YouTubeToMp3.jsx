...
function YouTubeConverter() {
	...
	async function handleConversion(formData) {
		const { id: videoId } = getVideoId(formData.videoUrl);
		...
		<Form onSubmit={handleSubmit(handleConversion)}>
			<InputGroup className="mb-3">
				...
				<Button
					...
					disabled={isLoading}
				>
					Convert <i className="bi bi-music-note" />
				</Button>
			</InputGroup>
			{errors.videoUrl && (
				...
				<Col xs={12} md={8} lg={6} className='mt-4 mt-md-5'>
					<Card className="shadow-sm">
						<Card.Header className="text-center bg-white border-bottom-0 pt-4">
							<div>
								<svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="currentColor" className="bi bi-youtube text-dark" viewBox="0 0 16 16">
								...
								<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" className="bi bi-music-note text-dark" viewBox="0 0 16 16">
						...
						<Card.Body>
							...
							{!isLoading && title && (
								<>
									{convertedUrl === '' &&
										<Alert className='my-20' variant="danger" onClose={() => setConvertedUrl('')}>
											Sorry can't not convert this video...
										</Alert>
									}
							...
							{progress === 100 && !isLoading && convertedUrl && (
								<div className='text-center p-3'>
									<Button href={convertedUrl} target="_blank" rel="noopener noreferrer" variant='dark'>Download <i className="bi bi-download ms-1" /></Button>
								</div>
							)}
